...
inline UIImage *MMSDScaledImageForKey(NSString *key, UIImage *image) {
    if (!image) {
        return nil;
    }
    ...
    if ([image.images count] > 0) {
        ...
        for (UIImage *tempImage in image.images) {
            [scaledImages addObject:MMSDScaledImageForKey(key, tempImage)];
        }
...
NSString *const MMSDWebImageErrorDomain = @"SDWebImageErrorDomain";
