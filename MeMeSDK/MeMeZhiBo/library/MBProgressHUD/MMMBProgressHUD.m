...
@implementation MMMBProgressHUD {
	BOOL useAnimation;
...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	MMMBProgressHUD *hud = [[self alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	MMMBProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	NSArray *huds = [MMMBProgressHUD allHUDsForView:view];
	for (MMMBProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
+ (MB_INSTANCETYPE)HUDForView:(UIView *)view {
	...
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:self]) {
			return (MMMBProgressHUD *)subview;
		}
	}
	return nil;
}
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue
	 completionBlock:(MMMBProgressHUDCompletionBlock)completion {
	self.taskInProgress = YES;
...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[MMMBRoundProgressView class]];
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		...
		[indicator removeFromSuperview];
        self.indicator = MB_AUTORELEASE([[MMMBBarProgressView alloc] init]);
		[self addSubview:indicator];
	}
		...
		if (mode == MBProgressHUDModeAnnularDeterminate) {
			[(MMMBRoundProgressView *)indicator setAnnular:YES];
		}
	} 
...
@implementation MMMBRoundProgressView
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		_mmbackgroundTintColor = [[UIColor alloc] initWithWhite:1.f alpha:.1f];
		[self registerForKVO];
	}
	return self;
}
...
- (void)drawRect:(CGRect)rect {
	...
	if (_annular) {
		...
		[_mmbackgroundTintColor setFill];
...
@implementation MMMBBarProgressView
