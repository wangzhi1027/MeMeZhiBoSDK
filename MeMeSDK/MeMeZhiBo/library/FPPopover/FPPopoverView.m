...
-(CGGradientRef)createGradient
{
    ...
    else if (self.tint == FPPopoverWhiteTint)
    {
        ...
        {
            colors[0] = colors[1] = colors[2] = 0.85;
            colors[4] = colors[5] = colors[6] = 0.9;
            colors[3] = colors[7] = 1.0;
        }
    }
...
- (void)drawRect:(CGRect)rect
{
    ...
    //content fill
    CGPathRef contentPath = [self createContentPathWithBorderWidth:0.5 arrowDirection:_arrowDirection];
    ...
    //external border
    CGPathRef externalBorderPath = [self createContentPathWithBorderWidth:0.5 arrowDirection:_arrowDirection];
    CGContextBeginPath(ctx);
...
//    //secondLine
//    cvRect.origin.x -= 1; cvRect.origin.y -= 1; cvRect.size.height += 1; cvRect.size.width += 1;
//    CGContextSetRGBStrokeColor(ctx, 0.4, 0.4, 0.4, 1.0);
//    CGContextStrokeRect(ctx, cvRect);
...
-(void)setupViews
{
    ...
    {
        // just now, only modify down-direction.
        _titleLabel.hidden = _titleHidden;
        if (_titleHidden)
        {
            contentRect.origin = CGPointMake(5, 5);
            contentRect.size = CGSizeMake(self.bounds.size.width-10, self.bounds.size.height-25);
            _titleLabel.frame = CGRectMake(5, 10, self.bounds.size.width-10, 20);
        }
        else
        {
            contentRect.origin = CGPointMake(5, 40);
            contentRect.size = CGSizeMake(self.bounds.size.width-10, self.bounds.size.height-60);
            _titleLabel.frame = CGRectMake(5, 10, self.bounds.size.width-10, 20);
        }
    }
