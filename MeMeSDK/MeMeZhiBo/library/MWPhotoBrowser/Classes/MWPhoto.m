        ...
        if (self.underlyingImage) {
            ...
            if (_image) {
                ...
                if ([[[_photoURL scheme] lowercaseString] isEqualToString:@"assets-library"]) {
                    ...
                    @try {
                        [[MMSDWebImageManager sharedManager] downloadWithURL:_photoURL options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize) {
...
- (void)imageLoadingComplete {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        _loadingInProgress = NO;
        [kNotificationCenter postNotificationName:MWPHOTO_LOADING_DID_END_NOTIFICATION
                                                            object:self];
    });
}
