...
- (void)_initialisation {
    ...
    // Defaults
    NSNumber *isVCBasedStatusBarAppearanceNum = [[NSBundle SDKResourcesBundle] objectForInfoDictionaryKey:@"UIViewControllerBasedStatusBarAppearance"];
...
- (void)dealloc {
    ...
    [[MMSDImageCache sharedImageCache] clearMemory]; // clear memory
}
...
- (void)viewDidLoad {
    ...
    // left back
    [self.uiManager.global setNavLeftBackItem:self action:@selector(goBack:) isBlack:YES];
    ...
    _toolbar.tintColor = [UIColor whiteColor];SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7") ? [UIColor whiteColor] : nil;
...
#else
        [self.uiManager.global setNavCustomUCBg:self];
#endif
    }
}
...
- (void)hideControlsAfterDelay {
	if (![self areControlsHidden]) {
        [self cancelControlHiding];
		_controlVisibilityTimer = [NSTimer scheduledTimerWithTimeInterval:15 target:self selector:@selector(hideControls) userInfo:nil repeats:NO];
	}
}
...
- (MMMBProgressHUD *)progressHUD {
    if (!_progressHUD) {
        _progressHUD = [[MMMBProgressHUD alloc] initWithView:self.view];
