...
- (void)insertSegmentWithTitle:(NSString *)title atIndex:(NSUInteger)segmentIndex animated:(BOOL)animated
{
    ...
    if (!segmentIndex) {
        segmentView.font = [UIFont boldSystemFontOfSize:18];
    }else{
        segmentView.font = [UIFont systemFontOfSize:18];
    }
    
    
//    CGRect orig = segmentView.frame;
//    segmentView.frame = CGRectMake(orig.origin.x, orig.origin.y+10, orig.size.width, orig.size.height);
    ...
    [self.items enumerateObjectsUsingBlock:^(UIView *item, NSUInteger idx, BOOL *stop)
     {
         item.alpha = 1;
             ...
             {
                 [item sizeToFit];
                 item.center = CGPointMake(pos + CGRectGetWidth(item.bounds) / 2, itemsVAlignCenter+10);
             }
             else
             {
                 item.frame = CGRectMake(pos, 0+10, CGRectGetWidth(item.bounds), itemsVAlignCenter * 2);
             }
                         ...
                         completion:^(BOOL finished)
         {
             for (UILabel *item in self.items)
             {
                 if (item == selectedItem)
                 {
                     ...
                     item.font = [UIFont boldSystemFontOfSize:18];
                 }
                 else
                 {
...
{
//    UIImage *tabBackground = [kImageNamed(@"img_tab_bg.png") resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0, 0, 2)];
//    self.layer.contents = (id)tabBackground.CGImage;
    
//    UIImageView* bgview = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"img_tab_bg.png"]];
